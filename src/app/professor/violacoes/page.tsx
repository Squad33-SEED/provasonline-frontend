...
import { getViolacoesProfessor, type PainelViolacoes } from "@/lib/violacoes";
...
export default function ViolacoesProfessorPage() {
  ...
  const carregarDados = React.useCallback(async () => {
    ...
    setErroCarga(null);
    try {
      setPainel(await getViolacoesProfessor());
    } catch (err) {
      const detail = err instanceof Error ? err.message : "Erro ao carregar violações";
      setErroCarga(detail);
      toast.push({
        variant: "destructive",
  ...
  return (
    <>
      <PageHeader
        title="Violações"
        description="Tentativas de cola detectadas nas etapas das suas turmas, por aluno"
      />
      ...
      <section className="grid grid-cols-2 gap-4 px-8 py-6">
        <Stat
          ...
          value={total}
          accent="rose"
          hint="Alunos das suas turmas"
        />
        <Stat
          label="Etapas afetadas"
          value={etapasAfetadas}
          accent="amber"
          hint="Com ao menos 1 ocorrência"
        />
      </section>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-white/5">
                {carregando ? (
                  ...
                  <tr>
                    <td colSpan={3} className="px-4 py-10 text-center text-sm text-white/40">
                      Nenhuma violação registrada nas suas turmas até o momento.
                    </td>
                  </tr>
                ) : (
                  porEtapa.map((e) => (
